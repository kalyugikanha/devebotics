...
// Fix 7: Row 1 — ALL white bold. Row 2 — ALL muted regular. Separator dot is #1F8844
...
const StatItem = ({ stat }: { stat: typeof stats[0] }) => {
  ...
  return (
    <div ref={elRef} style={{ textAlign: 'center' }}>
      <div style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 900, fontSize: 'clamp(48px, 6vw, 80px)', color: '#111827', lineHeight: 1, letterSpacing: '-0.03em' }}>
        {count}{stat.suffix}
      </div>
      <div style={{ fontFamily: 'Lexend, sans-serif', fontWeight: 300, fontSize: 13, color: '#4B5563', letterSpacing: '0.1em', textTransform: 'uppercase', marginTop: 8 }}>
        {stat.label}
      </div>
    </div>
  );
...
const Dot = () => (
  <span style={{ color: '#1F8844', fontSize: 8, lineHeight: 1, display: 'inline-flex', alignItems: 'center', padding: '0 14px' }}>●</span>
);

const Stats = () => (
  <section style={{ padding: '120px 0', background: '#F9FAFB', overflow: 'hidden' }}>
    {/* Count-up stats */}
    ...
    {/* Row 1 — ALL #111827, Montserrat SemiBold 18px, scrolls LEFT */}
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', animation: 'marquee 28s linear infinite', width: 'max-content' }}>
        {techRow.map((t, i) => (
          <span key={i} style={{ display: 'inline-flex', alignItems: 'center', whiteSpace: 'nowrap' }}>
            <span style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 600, fontSize: 18, color: '#111827' }}>{t}</span>
            <Dot />
          </span>
        ))}
      </div>
    ...
    {/* Row 2 — ALL #4B5563, Montserrat Regular 16px, scrolls RIGHT */}
    <div style={{
      overflow: 'hidden',
      maskImage: 'linear-gradient(to right, transparent, black 6%, black 94%, transparent)',
      WebkitMaskImage: 'linear-gradient(to right, transparent, black 6%, black 94%, transparent)',
      ...
      <div style={{ display: 'flex', alignItems: 'center', animation: 'marquee-rev 34s linear infinite', width: 'max-content' }}>
        {serviceRow.map((t, i) => (
          <span key={i} style={{ display: 'inline-flex', alignItems: 'center', whiteSpace: 'nowrap' }}>
            <span style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 400, fontSize: 16, color: '#4B5563' }}>{t}</span>
            <Dot />
          </span>
        ))}
      </div>
