...
const ContactForm = ({ preselectedApp = '' }: Props) => {
  ...
  const focusBorder = (e: React.FocusEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    e.target.style.borderColor = '#1F8844';
  };
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '0 32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '38% 1fr', gap: 80 }} className="contact-form-grid">
          ...
          <div className="fade-up">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20, marginBottom: 40 }}>
              {[
                ...
                <a key={c.label} href={c.href} style={{ display: 'flex', alignItems: 'center', gap: 16, textDecoration: 'none' }}>
                  <div style={{ width: 44, height: 44, background: '#F3F4F6', border: '1px solid #E5E7EB', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0 }}>{c.icon}</div>
                  <div>
                    <div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 11, color: '#9CA3AF', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 2 }}>{c.label}</div>
                    <div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 14, color: '#111827' }}>{c.val}</div>
                  </div>
                </a>
              ))}
            </div>
            ...
            {/* Response badge */}
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: '#F9FAFB', border: '1px solid #E5E7EB', borderRadius: 8, padding: '12px 18px', marginBottom: 32 }}>
              <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#1F8844', flexShrink: 0, display: 'inline-block' }} />
              <span style={{ fontFamily: 'Lexend, sans-serif', fontSize: 13, color: '#4B5563' }}>Average response time: Under 2 hours</span>
            </div>

            {/* Trust badges */}
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              {['🔒 NDA Protected', '⚡ Fast Delivery', '✅ Money-back'].map(b => (
                <span key={b} style={{ fontFamily: 'Lexend, sans-serif', fontSize: 12, color: '#4B5563', background: '#F3F4F6', border: '1px solid #E5E7EB', borderRadius: 6, padding: '7px 14px' }}>{b}</span>
              ))}
            </div>
          </div>
          ...
          <div className="fade-up">
            {submitted ? (
              <div style={{ textAlign: 'center', padding: '80px 0' }}>
                ...
                <h3 style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 24, color: '#111827', marginBottom: 12 }}>Message Sent!</h3>
                <p style={{ fontFamily: 'Lexend, sans-serif', fontSize: 15, color: '#4B5563' }}>We'll get back to you within 24 hours.</p>
              </div>
            ) : (
              <form onSubmit={e => { e.preventDefault(); setSubmitted(true); }} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <div>
                    <label style={{ fontFamily: 'Lexend, sans-serif', fontSize: 12, color: '#4B5563', display: 'block', marginBottom: 6 }}>Full Name *</label>
                  ...
                  <div>
                    <label style={{ fontFamily: 'Lexend, sans-serif', fontSize: 12, color: '#4B5563', display: 'block', marginBottom: 6 }}>Email Address *</label>
                ...
                <div>
                  <label style={{ fontFamily: 'Lexend, sans-serif', fontSize: 12, color: '#4B5563', display: 'block', marginBottom: 6 }}>Phone Number</label>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <div>
                    <label style={{ fontFamily: 'Lexend, sans-serif', fontSize: 12, color: '#4B5563', display: 'block', marginBottom: 6 }}>App Type *</label>
                  ...
                  <div>
                    <label style={{ fontFamily: 'Lexend, sans-serif', fontSize: 12, color: '#4B5563', display: 'block', marginBottom: 6 }}>Budget Range</label>
                ...
                <div>
                  <label style={{ fontFamily: 'Lexend, sans-serif', fontSize: 12, color: '#4B5563', display: 'block', marginBottom: 6 }}>App Description *</label>
                ...
                <p style={{ fontFamily: 'Lexend, sans-serif', fontWeight: 300, fontSize: 11, color: '#9CA3AF', textAlign: 'center' }}>
