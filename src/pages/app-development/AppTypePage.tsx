...
const steps = [
  { n: '01', t: 'Discovery & Scoping', d: 'We map your entire app flow, features, and technical requirements in detail.', img: 'https://images.unsplash.com/photo-1531538606174-0f90ff5dce83?w=500&q=80' },
  { n: '02', t: 'UI/UX Design', d: 'Clickable Figma prototype before a single line of code is written.', img: 'https://images.unsplash.com/photo-1561070791-2526d30994b5?w=500&q=80' },
  { n: '03', t: 'Development', d: 'Agile sprints. Clean code. Regular builds shared with you.', img: 'https://images.unsplash.com/photo-1498050108023-c5249f4df085?w=500&q=80' },
  { n: '04', t: 'QA & Testing', d: 'Manual + automated testing across 20+ real devices before launch.', img: 'https://images.unsplash.com/photo-1559136555-9303baea8ebd?w=500&q=80' },
  { n: '05', t: 'Launch & Submit', d: 'We handle Play Store & App Store submission, ASO & post-launch support.', img: 'https://images.unsplash.com/photo-1600880292089-90a7e086ee0c?w=500&q=80' },
];
...
export default function AppTypePage({ slug }: Props) {
  ...
  if (!data) return (
    <main style={{ background: '#08090A', minHeight: '100vh', paddingTop: 72, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 24, color: '#F0F0F0' }}>Page not found</div>
    </main>
  );
  ...
  return (
    <main style={{ background: '#08090A', minHeight: '100vh', paddingTop: 72 }}>
      ...
      <section style={{ minHeight: '90vh', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', padding: '120px 40px' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', textAlign: 'center', position: 'relative', zIndex: 1, opacity: 0, animation: 'fadeInUp 1s ease-out forwards' }}>
          ...
          <h1 style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 900, fontSize: 'clamp(48px, 8vw, 100px)', lineHeight: 0.95, letterSpacing: '-0.03em', marginBottom: 32, color: '#F0F0F0' }}>
            ...
            <span style={{ color: '#E8FF47', fontStyle: 'italic' }}>{data.heroAccentWord}</span>
          </h1>
        ...
        <div style={{ maxWidth: 1400, margin: '0 auto', padding: '0 32px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 0 }} className="features-grid">
            {data.overviewFeatures.map((f, i) => (
              <div key={i} className="fade-up" style={{ padding: '28px 32px 28px 0', borderBottom: '1px solid #1C1E21', borderRight: i % 3 !== 2 ? '1px solid #1C1E21' : 'none', paddingRight: i % 3 !== 2 ? 32 : 0, paddingLeft: i % 3 !== 0 ? 32 : 0, animationDelay: `${i * 0.08}s` }}>
                <div style={{ width: 32, height: 2, background: '#E8FF47', marginBottom: 16, borderRadius: 1 }} />
                <div style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 600, fontSize: 16, color: '#F0F0F0', marginBottom: 8 }}>{f.title}</div>
                <div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 13, color: '#6B6F76', lineHeight: 1.65 }}>{f.desc}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: '100px 0', background: '#08090A' }}>
        <div style={{ maxWidth: 1400, margin: '0 auto', padding: '0 32px' }}>
          <div className="fade-up" style={{ marginBottom: 56 }}>
            ...
            <p style={{ fontFamily: 'Lexend, sans-serif', fontSize: 16, color: '#6B6F76' }}>Complete ecosystem — not just one app.</p>
          </div>
          <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
            {data.panels.map((panel, i) => (
              <div key={panel} className="fade-up" style={{ flex: '1 1 180px', background: '#0E1012', border: '1px solid #1C1E21', borderRadius: 12, padding: '28px 24px', animationDelay: `${i * 0.08}s`, transition: 'border-color 0.2s ease' }}
                ...
                <div style={{ width: 36, height: 36, background: 'rgba(232,255,71,0.08)', borderRadius: 8, marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#E8FF47" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" /><path d="M3 9h18M9 21V9" /></svg>
                </div>
                <div style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 600, fontSize: 15, color: '#F0F0F0' }}>{panel}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* TECH STACK */}
      <section style={{ padding: '80px 0', background: '#0E1012' }}>
        <div style={{ maxWidth: 1400, margin: '0 auto', padding: '0 32px' }}>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {data.techStack.map(t => (
              <span key={t} className="fade-up" style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 500, fontSize: 14, color: '#6B6F76', background: '#08090A', border: '1px solid #1C1E21', borderRadius: 8, padding: '10px 18px', cursor: 'default', transition: 'border-color 0.2s ease, color 0.2s ease' }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = '#E8FF47'; (e.currentTarget as HTMLElement).style.color = '#F0F0F0'; }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = '#1C1E21'; (e.currentTarget as HTMLElement).style.color = '#6B6F76'; }}>
                {t}
              </span>
            ))}
          </div>
      ...
      <section style={{ padding: '100px 0', background: '#08090A' }}>
        <div style={{ maxWidth: 1400, margin: '0 auto', padding: '0 32px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 16 }} className="proc-grid">
            {steps.map((s, i) => (
              <div key={s.n} className="fade-up" style={{ background: '#0E1012', border: '1px solid #1C1E21', borderRadius: 12, overflow: 'hidden', transition: 'border-color 0.3s ease, transform 0.3s ease', animationDelay: `${i * 0.1}s` }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = '#E8FF47'; (e.currentTarget as HTMLElement).style.transform = 'translateY(-4px)'; }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = '#1C1E21'; (e.currentTarget as HTMLElement).style.transform = 'translateY(0)'; }}>
                <div style={{ position: 'relative', height: 140, overflow: 'hidden' }}>
                  <img src={s.img} alt={s.t} loading="lazy" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  <span style={{ position: 'absolute', top: 10, left: 10, fontFamily: 'Lexend, sans-serif', fontWeight: 600, fontSize: 10, background: '#E8FF47', color: '#08090A', padding: '3px 8px', borderRadius: 3 }}>STEP {s.n}</span>
                </div>
                <div style={{ padding: '14px 16px 18px' }}>
                  <div style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 900, fontSize: 48, color: '#111214', lineHeight: 1, marginBottom: -6 }}>{s.n}</div>
                  <div style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 800, fontSize: 14, color: '#F0F0F0', marginBottom: 6 }}>{s.t}</div>
                  <div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 11, color: '#6B6F76', lineHeight: 1.65 }}>{s.d}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {relatedCards.length > 0 && (
        <section id="portfolio" style={{ padding: '100px 0', background: '#08090A' }}>
          <div style={{ maxWidth: 1400, margin: '0 auto', padding: '0 32px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }} className="related-grid">
              {relatedCards.map(card => (
                <Link key={card.slug} to={`/app-development/${card.slug}`} style={{ textDecoration: 'none', background: '#0E1012', border: '1px solid #1C1E21', borderRadius: 12, overflow: 'hidden', display: 'block', transition: 'border-color 0.3s ease, transform 0.3s ease' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = '#E8FF47'; (e.currentTarget as HTMLElement).style.transform = 'translateY(-4px)'; }}
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = '#1C1E21'; (e.currentTarget as HTMLElement).style.transform = 'translateY(0)'; }}>
                  ...
                  <div style={{ padding: '18px 20px 22px' }}>
                    <div style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 800, fontSize: 17, color: '#F0F0F0', marginBottom: 6 }}>{card.name}</div>
                    <div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 13, color: '#6B6F76', marginBottom: 12 }}>{card.desc}</div>
                    <span style={{ fontFamily: 'Lexend, sans-serif', fontSize: 13, color: '#6B6F76' }}>Explore →</span>
                  </div>
                </Link>
              ))}
            </div>
            ...
            <div key={i} className="accordion-item fade-up" style={{ animationDelay: `${i * 0.05}s` }}>
              <button className="accordion-trigger" onClick={() => setOpenFaq(openFaq === i ? null : i)}>
                ...
                <svg width="18" height="18" viewBox="0 0 18 18" fill="none" style={{ flexShrink: 0, transition: 'transform 0.2s ease', transform: openFaq === i ? 'rotate(45deg)' : 'rotate(0deg)' }}>
                  <path d="M9 4v10M4 9h10" stroke="#E8FF47" strokeWidth="1.5" strokeLinecap="round" />
                </svg>
              </button>
